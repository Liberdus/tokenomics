    ...
    question: "What is the token distribution for LIB?",
     answer: (
      <>
        ...
        <a href="/emissions" className="text-blue-500 underline">
          here
        </a>.
      </>
    ),
...
export default function FAQ() {
  // Start with the current hash (if any)
  ...
  // Optional: Automatically scroll to the FAQ with the matching hash
  useEffect(() => {
    if (currentHash) {
        ...
        <div className="mx-auto max-w-4xl divide-y-4 divide-purple">
          ...
          <dl className="mt-10 space-y-6 divide-y divide-purple">
            {faqs.map((faq, index) => {
              ...
              const shouldOpen = currentHash === `#${faqId}`
              // Changing the key whenever currentHash changes will force the Disclosure to re-mount,
              // so that defaultOpen takes effect.
              return (
                <Disclosure key={`${faq.question}-${currentHash}`} defaultOpen={shouldOpen} as="div" className="pt-6">
